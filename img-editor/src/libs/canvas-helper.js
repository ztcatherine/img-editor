...
let getColorByCoord = function (imgData, {
  x,
  y
}) {
  let n, r, g, b, a
...
let setColorByCoord = function (imgData, {
  r,
  g,
  b,
  a
...
export default class Ctx {
  ...
  //   }
  // }
  url(x = 0, y = 0, w = this.w, h = this.h,left,top,width,height,imgUrl) {
    debugger
    var img = new Image();
    img.src = imgUrl
    if (x == 0 && y == 0 && w == this.w && h == this.h) {
  ...
  mosaic(strength = 5, x = 0, y = 0, w = this.w, h = this.h) {
    ...
    while (i < d.length) {
      ...
      mx = X - (X % strength)
      my = Y - (Y % strength)
      c = getColorByCoord(imgData, {
        x: mx,
        y: my
      })
      setColorByCoord(imgData, c, {
        x: X,
        y: Y
      })
      i += 4
